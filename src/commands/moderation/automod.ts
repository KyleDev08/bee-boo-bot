...
export default {
        ...
        .addStringOption((opt) =>
          opt
            ...
            .addChoices(
              ...
              { name: "timeout de 10 minuto", value: "timeout" },
              { name: "Bloqu. + Alerta", value: "block_alert" },
              { name: "Alerta + Timeout", value: "alert_timeout" },
              { name: "Las 3 acciones", value: "all" }
            )
        )
    )
    .addSubcommand((subC) =>
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!logChannel) {
      return interaction.reply({
        content:
          "Automod log channel is not set up. Please set it up using /automod setup.",
        flags: ctxResponseFlags.Ephemeral,
      });
    }
