import { SlashCommandBuilder, MessageFlags as ctxResponseFlags, InteractionContextType, EmbedBuilder, ChannelType, ButtonBuilder, ButtonStyle, ActionRowBuilder, } from "discord.js";
...
import { addOverwriteCheck, removeOverwriteCheck, } from "../../functions/verifyOverwrites.js";
import { checkBotPermissionsInCategory, checkBotPermissionsInChannel, } from "../../functions/checkPermissions.js";
export default {
    data: new SlashCommandBuilder()
        ...
        .setRequired(false)))
        .addSubcommand((subc) => subc.setName("list").setDescription("List all open tickets"))
    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === "setup") {
            ...
            if (categoryId && roleId) {
                ...
                await ticketCounter.findOneAndUpdate({ guildId: interaction.guild?.id }, {
                    $set: {
            ...
            const channelEmbed = await interaction.guild.channels.fetch(channelId || interaction.channelId);
            ...
            const ticketButton = new ButtonBuilder()
                ...
                .setEmoji("🎫");
            const rowTicket = new ActionRowBuilder().addComponents(ticketButton);
        ...
        if (interaction.options.getSubcommand() === "create") {
            ...
            const hasPermission = interaction.memberPermissions?.has("ManageMessages");
            const guildDoc = await ticketCounter.findOneAndUpdate({ guildId: interaction.guild?.id }, {
                $inc: { count: 1 },
                $setOnInsert: { guildId: interaction.guild?.id },
            }, { new: true, upsert: true });
            ...
            if (targetUser) {
                ...
                if (targetUser.id === interaction.user.id) {
                    ...
                    if (hasPermission) {
                        ...
                        return interaction.reply({
                            content: "❌ No tienes permiso para abrir un ticket a otro usuario.",
                            flags: ctxResponseFlags.Ephemeral,
                        });
                    }
                }
            ...
            if (ticketGuildDoc) {
                ...
                if (channel) {
                    await safeDM(user, { content: `Ya tienes un ticket abierto: ${channel}` }, interaction);
        ...
        if (interaction.options.getSubcommand() === "add") {
            ...
            const hasPermission = interaction.memberPermissions?.has("ManageChannels");
            if (!hasPermission) {
                return interaction.reply({
                    content: "❌ No tienes permiso para usar este comando.",
                    flags: ctxResponseFlags.Ephemeral,
            ...
            await addOverwriteCheck(channel, interaction, user?.id, role?.id).then(async (exists) => {
                if (exists)
                    return;
            });
            if (role && user) {
        ...
        if (interaction.options.getSubcommand() === "remove") {
            ...
            await removeOverwriteCheck(channel, interaction, user?.id, role?.id).then(async (exists) => {
                if (exists)
                    return;
            });
            if (role && user) {
