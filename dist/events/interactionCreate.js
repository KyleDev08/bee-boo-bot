    ...
    async execute(interaction) {
        if (interaction.isButton()) {
            if (interaction.customId === "create_ticket") {
                ...
                if (ticketGuildDoc) {
                    const channel = await interaction.guild?.channels.fetch(ticketGuildDoc.channelId);
                    if (channel) {
                        try {
                            await user.send({
            ...
            if (interaction.customId === "transcript_ticket") {
                ...
                const transcript = msg.map((message) => {
                    ...
                    if (message.stickers.size > 0) {
                        const stickers = message.stickers.map((sticker) => `[Sticker: ${sticker.name}] (${sticker.url})`);
                        parts.push(...stickers);
                    }
                    if (message.attachments.size > 0) {
                        const files = message.attachments.map((file) => `[Archivo: ${file.name}] (${file.url})`);
                        parts.push(...files);
                    }
                    if (message.reactions.cache.size > 0) {
                        const reactions = message.reactions.cache.map((r) => `${r.emoji.name} x${r.count}`);
                ...
                const safeTranscript = transcriptText.length > MAX_LENGTH
